...
> [!WARNING]
> Riot Vanguard 已更新其验证机制，当前用户级注入方式已受到影响。系统开机后通常仅首次注入可以成功，后续在不重启电脑的情况下再次注入可能失败。
>
> 如果 Riot Vanguard 后续继续调整相关验证逻辑，本项目可能进一步失效，且不保证会持续适配或维护。

> [!IMPORTANT]
> 请勿在 Issues 中发布广告、引流或重复推广同类付费产品。相关产品是否可用与本项目无关，但辱骂、刷屏或使用小号反复发布的行为不会被接受。
